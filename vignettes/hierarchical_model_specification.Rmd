...
The core idea of `paramedic` is to combine estimates of absolute and relative abundance by modeling efficiency. In [our paper](https://www.biorxiv.org/content/10.1101/761486v1), we detailed a potential hierarchical model that incorporates efficiency. We also showed how the model can be extended to include covariates. In this vignette, we explore the other options currently available in `paramedic` and provide guidelines for requesting new hierarchical model specifications. In all examples below, we use the example data referenced in the [main vignette](introduction_to_paramedic.html), so we load those data now. Additionally, we use a *very* small number of total and burn-in iterations for illustration only; we stress that a larger number of iterations should be used in practice.
